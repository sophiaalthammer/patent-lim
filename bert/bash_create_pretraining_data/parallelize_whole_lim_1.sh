...
foo () {
   ...
   # source activate tensorflow_p27
    ...
    python create_pretraining_data_lim.py   --input_file=./data/part-000000000$run.txt --input_file_np=./data/np_vectors_part-000000000$run.txt  --output_file=./data/tfrecords_seq_length128/tfrecord_128_lim_1_part-000000000$run.tfrecord --lim_prob=1.0 --vocab_file=$BERT_BASE_DIR/vocab.txt   --do_lower_case=False   --max_seq_length=128   --max_predictions_per_seq=20   --masked_lm_prob=0.15   --random_seed=12345 --dupe_factor=5
     aws s3 cp /home/ubuntu/bert/data/tfrecords_seq_length128/tfrecord_128_lim_1_part-000000000$run.tfrecord s3://aws-s3-patent/tfrecords_lim/tfrecords_lim_plim_1/tfrecord_128_lim_1_part-000000000$run.tfrecord
    rm data/part-000000000$run.txt
    rm data/np_vectors_part-000000000$run.txt
    rm data/tfrecords_seq_length128/tfrecord_128_lim_1_part-000000000$run.tfrecord
