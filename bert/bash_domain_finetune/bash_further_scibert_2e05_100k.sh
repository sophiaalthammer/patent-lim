...
wait
export BERT_BASE_DIR=/home/ubuntu/bert
python run_pretraining.py --input_file=$DESTINATIONS  --output_dir=$BERT_BASE_DIR/output/scibert_mlm   --do_train=True   --do_eval=True   --bert_config_file=$BERT_BASE_DIR/scibert_config.json   --init_checkpoint=$BERT_BASE_DIR/scibert_model.ckpt   --save_checkpoints_steps=5000   --train_batch_size=32   --max_seq_length=128   --max_predictions_per_seq=20   --num_train_steps=100000   --num_warmup_steps=10000 --learning_rate=2e-5
wait
aws s3 cp --recursive /home/ubuntu/bert/output/scibert_mlm/ s3://aws-s3-patent/models/scibert/further_2e05_100k/
wait
