...
foo () {
    local run=$1
    aws s3 cp s3://aws-s3-patent/downstream_tasks/ipc_classification/data/tfrecords/ipc_claim_scibert_train_0$run.tfrecord /home/ubuntu/bert/data/ipc_claim_scibert_train_0$run.tfrecord
}
for run in {000..030}; do foo "$run" & done
wait
for run in {031..061}; do foo "$run" & done
wait
for run in {062..093}; do foo "$run" & done
wait
for run in {094..125}; do foo "$run" & done
wait
################## EVALUATION/DEV SAMPLES
foo2 () {
    local run=$1
    aws s3 cp s3://aws-s3-patent/downstream_tasks/ipc_classification/data/tfrecords/ipc_claim_scibert_dev_0$run.tfrecord /home/ubuntu/bert/data/ipc_claim_scibert_dev_0$run.tfrecord
}
...
################## Bert Vanilla 
python run_classifier.py  --task_name=IPC  --do_train=true  --do_eval=true --input_train_file=$DESTINATIONS --input_eval_file=$DESTINATIONS2  --data_dir=$DATA_DIR  --vocab_file=$BERT_BASE_DIR/scibert_vocab.txt  --bert_config_file=$BERT_BASE_DIR/scibert_config.json --init_checkpoint=$BERT_BASE_DIR/scibert_model.ckpt  --save_checkpoints_steps=5000 --max_seq_length=128  --train_batch_size=32   --learning_rate=5e-5  --num_train_epochs=1.0 --num_train_steps=60000  --do_lower_case=False   --label_location=$DATA_DIR/ipc_all_unique_tags_files_000-125_180-200.pkl  --output_dir=$BERT_BASE_DIR/output/ipc_classification/scibert_vanilla/
wait
...
wait
################## Bert MLM
python run_classifier.py  --task_name=IPC  --do_train=true  --do_eval=true --input_train_file=$DESTINATIONS --input_eval_file=$DESTINATIONS2  --data_dir=$DATA_DIR  --vocab_file=$BERT_BASE_DIR/scibert_vocab.txt  --bert_config_file=$BERT_BASE_DIR/scibert_config.json --init_checkpoint=$BERT_BASE_DIR/model_scibert_mlm.ckpt-100000 --save_checkpoints_steps=5000  --max_seq_length=128  --train_batch_size=32   --learning_rate=5e-5  --num_train_epochs=1.0 --num_train_steps=60000  --do_lower_case=False   --label_location=$DATA_DIR/ipc_all_unique_tags_files_000-125_180-200.pkl  --output_dir=$BERT_BASE_DIR/output/ipc_classification/scibert_mlm/
wait
...
wait
################## Bert LIM1
python run_classifier.py  --task_name=IPC  --do_train=true  --do_eval=true --input_train_file=$DESTINATIONS --input_eval_file=$DESTINATIONS2  --data_dir=$DATA_DIR  --vocab_file=$BERT_BASE_DIR/scibert_vocab.txt  --bert_config_file=$BERT_BASE_DIR/scibert_config.json --init_checkpoint=$BERT_BASE_DIR/model_scibert_lim1.ckpt-100000 --save_checkpoints_steps=5000  --max_seq_length=128  --train_batch_size=32   --learning_rate=5e-5  --num_train_epochs=1.0 --num_train_steps=60000  --do_lower_case=False   --label_location=$DATA_DIR/ipc_all_unique_tags_files_000-125_180-200.pkl  --output_dir=$BERT_BASE_DIR/output/ipc_classification/scibert_lim1/
wait
...
wait
################## Bert LIM075
python run_classifier.py  --task_name=IPC  --do_train=true  --do_eval=true --input_train_file=$DESTINATIONS --input_eval_file=$DESTINATIONS2  --data_dir=$DATA_DIR  --vocab_file=$BERT_BASE_DIR/scibert_vocab.txt  --bert_config_file=$BERT_BASE_DIR/scibert_config.json --init_checkpoint=$BERT_BASE_DIR/model_scibert_lim075.ckpt-100000 --save_checkpoints_steps=5000 --max_seq_length=128  --train_batch_size=32   --learning_rate=5e-5  --num_train_epochs=1.0 --num_train_steps=60000  --do_lower_case=False   --label_location=$DATA_DIR/ipc_all_unique_tags_files_000-125_180-200.pkl  --output_dir=$BERT_BASE_DIR/output/ipc_classification/scibert_lim075/
wait
